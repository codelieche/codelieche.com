...
def get_hot_article_tags():
    ...
    :return: 返回所有文章的标签
    """
    return Tag.objects.filter(is_hot=True)
...
def get_latest_posts(count=5):
    ...
    @register.inclusion_tag('article/latest_posts.html')
    """
    latest_posts = Post.published.order_by('-time_added')[:count]
...
def get_similar_posts(post,count=5):
    ...
    similar_posts = similar_posts.annotate(same_tags=Count('id')).order_by(
        '-same_tags', '-time_added')[:count]
