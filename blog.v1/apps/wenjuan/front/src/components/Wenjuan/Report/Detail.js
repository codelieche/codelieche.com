...
class ReportDetail extends Component {
    ...
    fetchDetailData = () => {
        const url = "http://127.0.0.1:8090/api/v1/wenjuan/report/" + this.state.id;
    ...
    fetchJobDetailData = (id) => {
        const url = "http://127.0.0.1:8090/api/v1/wenjuan/job/" + id;
        fetch(url, {method: "GET", credentials: "include"})
           .then(response => response.json())
           .then(data => {
               if(data.id){
    ...
    render() {
        return (
            <Layout className="wenjuan">
                ...
                <Content>
                    <div className="container wenjuan wenjuan-report">
